...
namespace Web_Services.Controllers
{
    ...
    public class BookingController : Controller
    {
        ...
        public async Task<dynamic> CreateReservation(string id, string name,
       string time, string start, string departure, [FromBody] Auth user)
        {
            ...
            await _mongoDBService.CreateReservation(id, name, time, start,
           departure, user);
        ...
        //Create Assistant Requesu
        [HttpPost("assistance")]
        public async Task<dynamic> CreateAssistance([FromBody] Assistance
       assistance)
        {
            var request = Request;
            if (await EventMiddleware.Authorizer(request.Headers["x-api-key"].IsNullOrEmpty() ? null : request.Headers["x-api-key"][0], _mongoDBService))
            ...
            return Ok("Create Assitance Successfully");
        }
        //get assistant
